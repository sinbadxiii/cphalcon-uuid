...
{
	ZEPHIR_REGISTER_CLASS(Phalcon2\\Encryption\\Security\\Uuid, SysNodeProvider, phalcon2, encryption_security_uuid_sysnodeprovider, phalcon2_encryption_security_uuid_sysnodeprovider_method_entry, 0);

	zephir_declare_class_constant_string(phalcon2_encryption_security_uuid_sysnodeprovider_ce, SL("IFCONFIG_PATTERN"), "/[^:]([0-9a-f]{2}([:-])[0-9a-f]{2}(\2[0-9a-f]{2}){4})[^:]/i");

	zend_class_implements(phalcon2_encryption_security_uuid_sysnodeprovider_ce, 1, phalcon2_encryption_security_uuid_nodeproviderinterface_ce);
	return SUCCESS;
}
...
PHP_METHOD(Phalcon2_Encryption_Security_Uuid_SysNodeProvider, getNode)
{
	...
	if (ZEPHIR_IS_STRING_IDENTICAL(&_0, "LINUX")) {
		...
		if (_3$$3) {
			...
			array_init(&macs$$5);
			zephir_is_iterable(&addressPaths$$3, 0, "phalcon2/Encryption/Security/Uuid/SysNodeProvider.zep", 29);
			if (Z_TYPE_P(&addressPaths$$3) == IS_ARRAY) {
				ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(&addressPaths$$3), _4$$5)
				{
					...
					if (zephir_is_true(&_6$$6)) {
						...
						zephir_array_append(&macs$$5, &_8$$7, PH_SEPARATE, "phalcon2/Encryption/Security/Uuid/SysNodeProvider.zep", 25);
					}
				...
				while (1) {
					...
					zephir_check_call_status();
						...
						if (zephir_is_true(&_9$$8)) {
							...
							zephir_array_append(&macs$$5, &_10$$9, PH_SEPARATE, "phalcon2/Encryption/Security/Uuid/SysNodeProvider.zep", 25);
						}
			...
			ZEPHIR_INIT_NVAR(&_11$$5);
			zephir_create_closure_ex(&_11$$5, NULL, phalcon2_0__closure_ce, SL("__invoke"));
	...
	if (ZEPHIR_IS_STRING_IDENTICAL(&node, "")) {
		...
		if (zephir_is_true(&_29$$10)) {
			zephir_array_fetch_long(&_32$$16, &matches$$10, 1, PH_NOISY | PH_READONLY, "phalcon2/Encryption/Security/Uuid/SysNodeProvider.zep", 72);
			zephir_is_iterable(&_32$$16, 0, "phalcon2/Encryption/Security/Uuid/SysNodeProvider.zep", 77);

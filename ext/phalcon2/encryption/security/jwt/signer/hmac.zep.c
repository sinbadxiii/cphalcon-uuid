...
{
	ZEPHIR_REGISTER_CLASS_EX(Phalcon2\\Encryption\\Security\\JWT\\Signer, Hmac, phalcon2, encryption_security_jwt_signer_hmac, phalcon2_encryption_security_jwt_signer_abstractsigner_ce, phalcon2_encryption_security_jwt_signer_hmac_method_entry, 0);
...
PHP_METHOD(Phalcon2_Encryption_Security_JWT_Signer_Hmac, __construct)
{
	...
	if (!(zephir_array_isset(&supported, &algo))) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(phalcon2_encryption_security_jwt_exceptions_unsupportedalgorithmexception_ce, "Unsupported HMAC algorithm", "phalcon2/Encryption/Security/JWT/Signer/Hmac.zep", 40);
		return;
	}
...
PHP_METHOD(Phalcon2_Encryption_Security_JWT_Signer_Hmac, sign)
{
	...
	ZEPHIR_RETURN_CALL_METHOD(this_ptr, "gethash", NULL, 47, &payload, &passphrase);
...
PHP_METHOD(Phalcon2_Encryption_Security_JWT_Signer_Hmac, verify)
{
	...
	ZEPHIR_CALL_METHOD(&_0, this_ptr, "gethash", NULL, 47, &payload, &passphrase);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon2_Encryption_Security_JWT_Signer_Hmac, getHash)
{
	...
	zephir_check_call_status();
	ZEPHIR_RETURN_CALL_FUNCTION("hash_hmac", NULL, 6, &_0, &payload, &passphrase, &__$true);
	zephir_check_call_status();
	RETURN_MM();
}

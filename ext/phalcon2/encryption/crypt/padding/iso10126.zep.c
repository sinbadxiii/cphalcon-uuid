...
{
	ZEPHIR_REGISTER_CLASS(Phalcon2\\Encryption\\Crypt\\Padding, Iso10126, phalcon2, encryption_crypt_padding_iso10126, phalcon2_encryption_crypt_padding_iso10126_method_entry, 0);

	zend_class_implements(phalcon2_encryption_crypt_padding_iso10126_ce, 1, phalcon2_encryption_crypt_padding_padinterface_ce);
	return SUCCESS;
}
...
PHP_METHOD(Phalcon2_Encryption_Crypt_Padding_Iso10126, pad)
{
	...
	if (_1 <= _2) {
		while (1) {
			...
			ZEPHIR_CALL_FUNCTION(&_3$$3, "rand", &_4, 45);
			zephir_check_call_status();
			ZEPHIR_CALL_FUNCTION(&_5$$3, "chr", &_6, 42, &_3$$3);
			zephir_check_call_status();
			zephir_concat_self(&padding, &_5$$3);
		}
	}
	ZVAL_LONG(&_7, paddingSize);
	ZEPHIR_CALL_FUNCTION(&_8, "chr", &_6, 42, &_7);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon2_Encryption_Crypt_Padding_Iso10126, unpad)
{
	...
	ZEPHIR_RETURN_CALL_FUNCTION("ord", NULL, 44, &last);
	zephir_check_call_status();
	RETURN_MM();
}

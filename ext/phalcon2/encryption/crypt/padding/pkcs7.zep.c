...
{
	ZEPHIR_REGISTER_CLASS(Phalcon2\\Encryption\\Crypt\\Padding, Pkcs7, phalcon2, encryption_crypt_padding_pkcs7, phalcon2_encryption_crypt_padding_pkcs7_method_entry, 0);

	zend_class_implements(phalcon2_encryption_crypt_padding_pkcs7_ce, 1, phalcon2_encryption_crypt_padding_padinterface_ce);
	return SUCCESS;
}
...
PHP_METHOD(Phalcon2_Encryption_Crypt_Padding_Pkcs7, unpad)
{
	...
	if (ZEPHIR_LE_LONG(&ord, blockSize)) {
		...
		ZEPHIR_CALL_FUNCTION(&_2$$3, "chr", NULL, 42, &paddingSize);
		zephir_check_call_status();
		ZEPHIR_CALL_FUNCTION(&padding, "str_repeat", NULL, 43, &_2$$3, &paddingSize);
		zephir_check_call_status();

...
{
	ZEPHIR_REGISTER_CLASS(Phalcon2\\Encryption\\Crypt\\Padding, Space, phalcon2, encryption_crypt_padding_space, phalcon2_encryption_crypt_padding_space_method_entry, 0);

	zend_class_implements(phalcon2_encryption_crypt_padding_space_ce, 1, phalcon2_encryption_crypt_padding_padinterface_ce);
	return SUCCESS;
}
...
PHP_METHOD(Phalcon2_Encryption_Crypt_Padding_Space, pad)
{
	...
	ZEPHIR_RETURN_CALL_FUNCTION("str_repeat", NULL, 43, &_0, &_1);
...
PHP_METHOD(Phalcon2_Encryption_Crypt_Padding_Space, unpad)
{
	...
	while (1) {
		_0 = counter >= 0;
		if (_0) {
			zephir_array_fetch_long(&_1, &inputArray, counter, PH_NOISY | PH_READONLY, "phalcon2/Encryption/Crypt/Padding/Space.zep", 48);
			ZVAL_LONG(&_2, 32);

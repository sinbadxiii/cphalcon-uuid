...
{
	ZEPHIR_REGISTER_CLASS(Phalcon2\\Encryption\\Crypt\\Padding, Zero, phalcon2, encryption_crypt_padding_zero, phalcon2_encryption_crypt_padding_zero_method_entry, 0);

	zend_class_implements(phalcon2_encryption_crypt_padding_zero_ce, 1, phalcon2_encryption_crypt_padding_padinterface_ce);
	return SUCCESS;
}
...
PHP_METHOD(Phalcon2_Encryption_Crypt_Padding_Zero, unpad)
{
	...
	while (1) {
		_0 = counter >= 0;
		if (_0) {
			zephir_array_fetch_long(&_1, &inputArray, counter, PH_NOISY | PH_READONLY, "phalcon2/Encryption/Crypt/Padding/Zero.zep", 48);
			ZVAL_LONG(&_2, 0);

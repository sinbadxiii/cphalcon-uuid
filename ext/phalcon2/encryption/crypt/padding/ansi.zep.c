...
{
	ZEPHIR_REGISTER_CLASS(Phalcon2\\Encryption\\Crypt\\Padding, Ansi, phalcon2, encryption_crypt_padding_ansi, phalcon2_encryption_crypt_padding_ansi_method_entry, 0);

	zend_class_implements(phalcon2_encryption_crypt_padding_ansi_ce, 1, phalcon2_encryption_crypt_padding_padinterface_ce);
	return SUCCESS;
}
...
PHP_METHOD(Phalcon2_Encryption_Crypt_Padding_Ansi, pad)
{
	...
	ZEPHIR_CALL_FUNCTION(&_2, "str_repeat", NULL, 43, &_1, &_0);
	zephir_check_call_status();
	ZVAL_LONG(&_0, paddingSize);
	ZEPHIR_CALL_FUNCTION(&_3, "chr", NULL, 42, &_0);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon2_Encryption_Crypt_Padding_Ansi, unpad)
{
	...
	ZEPHIR_CALL_FUNCTION(&ord, "ord", NULL, 44, &last);
	zephir_check_call_status();
	if (ZEPHIR_LE_LONG(&ord, blockSize)) {
		...
		if (ZEPHIR_GT_LONG(&paddingSize, 1)) {
			ZVAL_LONG(&_2$$4, 0);
			ZEPHIR_CALL_FUNCTION(&_3$$4, "chr", NULL, 42, &_2$$4);
			zephir_check_call_status();
			ZVAL_LONG(&_2$$4, (zephir_get_numberval(&paddingSize) - 1));
			ZEPHIR_CALL_FUNCTION(&repeat, "str_repeat", NULL, 43, &_3$$4, &_2$$4);
			zephir_check_call_status();
		}

...
{
	ZEPHIR_REGISTER_CLASS_EX(Phalcon\\Encryption, Security, phalcon, encryption_security, zephir_get_internal_ce(SL("phalcon\\di\\abstractinjectionaware")), phalcon_encryption_security_method_entry, 0);
...
PHP_METHOD(Phalcon_Encryption_Security, checkHash)
{
	...
	ZEPHIR_RETURN_CALL_FUNCTION("password_verify", NULL, 30, &password, &passwordHash);
	zephir_check_call_status();
	RETURN_MM();
}
...
PHP_METHOD(Phalcon_Encryption_Security, checkToken)
{
	...
	ZEPHIR_CALL_METHOD(&_0, this_ptr, "processtokenkey", NULL, 31, &tokenKey);
	zephir_check_call_status();
	zephir_get_strval(&tokenKey, &_0);
	if (!(!(ZEPHIR_IS_EMPTY(&tokenKey)))) {
		RETURN_MM_BOOL(0);
	}
	ZEPHIR_CALL_METHOD(&userToken, this_ptr, "processusertoken", NULL, 32, &tokenKey, tokenValue);
	zephir_check_call_status();
...
PHP_METHOD(Phalcon_Encryption_Security, computeHmac)
{
	...
	ZEPHIR_CALL_FUNCTION(&hmac, "hash_hmac", NULL, 6, &algo, &data, &key, &_0);
	zephir_check_call_status();
	if (UNEXPECTED(!zephir_is_true(&hmac))) {
		...
		ZEPHIR_CALL_FUNCTION(&_3$$3, "sprintf", NULL, 15, &_2$$3, &algo);
		zephir_check_call_status();
		ZEPHIR_CALL_METHOD(NULL, &_1$$3, "__construct", NULL, 16, &_3$$3);
		zephir_check_call_status();
...
PHP_METHOD(Phalcon_Encryption_Security, getHashInformation)
{
	...
	ZEPHIR_RETURN_CALL_FUNCTION("password_get_info", NULL, 33, &hash);
	zephir_check_call_status();
	RETURN_MM();
}
...
PHP_METHOD(Phalcon_Encryption_Security, hash)
{
	...
	ZEPHIR_CALL_METHOD(&cost, this_ptr, "processcost", NULL, 34, &options);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(&_0);
	ZVAL_STRING(&_0, "%02s");
	ZEPHIR_CALL_FUNCTION(&formatted, "sprintf", NULL, 15, &_0, &cost);
	zephir_check_call_status();
	...
	do {
		...
		if (ZEPHIR_IS_LONG(&_1, 5)) {
			...
			ZEPHIR_CALL_FUNCTION(&prefix, "sprintf", NULL, 15, &_2$$6, &formatted);
			zephir_check_call_status();
			break;
		}
		if (ZEPHIR_IS_LONG(&_1, 6)) {
			...
			ZEPHIR_CALL_FUNCTION(&prefix, "sprintf", NULL, 15, &_3$$7, &formatted);
			zephir_check_call_status();
			break;
		}
		legacy = 0;
	...
	if (UNEXPECTED(legacy)) {
		...
		ZEPHIR_RETURN_CALL_FUNCTION("crypt", NULL, 35, &password, &salt);
		zephir_check_call_status();
		RETURN_MM();
	}
	ZEPHIR_INIT_VAR(&_6);
	...
	ZEPHIR_CALL_METHOD(&algorithm, this_ptr, "processalgorithm", NULL, 36);
	zephir_check_call_status();
	ZEPHIR_CALL_METHOD(&arguments, this_ptr, "processargonoptions", NULL, 37, &options);
	zephir_check_call_status();
	ZEPHIR_RETURN_CALL_FUNCTION("password_hash", NULL, 38, &password, &algorithm, &arguments);
	zephir_check_call_status();
	RETURN_MM();
}

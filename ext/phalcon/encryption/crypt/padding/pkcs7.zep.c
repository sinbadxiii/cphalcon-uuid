...
PHP_METHOD(Phalcon_Encryption_Crypt_Padding_Pkcs7, pad)
{
	...
	ZVAL_LONG(&_0, paddingSize);
	ZEPHIR_RETURN_CALL_FUNCTION("str_repeat", NULL, 26, &_1, &_0);
	zephir_check_call_status();
	RETURN_MM();
}
...
PHP_METHOD(Phalcon_Encryption_Crypt_Padding_Pkcs7, unpad)
{
	...
	ZEPHIR_CALL_FUNCTION(&ord, "ord", NULL, 27, &last);
	zephir_check_call_status();
	if (ZEPHIR_LE_LONG(&ord, blockSize)) {
		ZEPHIR_CPY_WRT(&paddingSize, &ord);
		ZEPHIR_CALL_FUNCTION(&_2$$3, "chr", NULL, 25, &paddingSize);
		zephir_check_call_status();
		ZEPHIR_CALL_FUNCTION(&padding, "str_repeat", NULL, 26, &_2$$3, &paddingSize);
		zephir_check_call_status();

...
{
	ZEPHIR_REGISTER_CLASS(Sparrow\\Encryption, Crypt, sparrow, encryption_crypt, sparrow_encryption_crypt_method_entry, 0);

	/**
	 * @var string
	 */
	zend_declare_property_string(sparrow_encryption_crypt_ce, SL("authData"), "", ZEND_ACC_PROTECTED);
	/**
	 * @var string
	 */
	zend_declare_property_string(sparrow_encryption_crypt_ce, SL("authTag"), "", ZEND_ACC_PROTECTED);
	/**
	 * @var int
	 */
	zend_declare_property_long(sparrow_encryption_crypt_ce, SL("authTagLength"), 16, ZEND_ACC_PROTECTED);
	/**
	 * Available cipher methods.
	 *
	 * @var array
	 */
	zend_declare_property_null(sparrow_encryption_crypt_ce, SL("availableCiphers"), ZEND_ACC_PROTECTED);
	/**
	 * @var string
	 */
	zend_declare_property_string(sparrow_encryption_crypt_ce, SL("cipher"), "aes-256-cfb", ZEND_ACC_PROTECTED);
	/**
	 * The name of hashing algorithm.
	 *
	 * @var string
	 */
	zend_declare_property_string(sparrow_encryption_crypt_ce, SL("hashAlgorithm"), "sha256", ZEND_ACC_PROTECTED);
	/**
	 * The cipher iv length.
	 *
	 * @var int
	 */
	zend_declare_property_long(sparrow_encryption_crypt_ce, SL("ivLength"), 16, ZEND_ACC_PROTECTED);
	/**
	 * @var string
	 */
	zend_declare_property_string(sparrow_encryption_crypt_ce, SL("key"), "", ZEND_ACC_PROTECTED);
	/**
	 * @var int
	 */
	zend_declare_property_long(sparrow_encryption_crypt_ce, SL("padding"), 0, ZEND_ACC_PROTECTED);
	/**
	 * @var PadFactory
	 */
	zend_declare_property_null(sparrow_encryption_crypt_ce, SL("padFactory"), ZEND_ACC_PROTECTED);
	/**
	 * Whether calculating message digest enabled or not.
	 *
	 * @var bool
	 */
	zend_declare_property_bool(sparrow_encryption_crypt_ce, SL("useSigning"), 1, ZEND_ACC_PROTECTED);
	sparrow_encryption_crypt_ce->create_object = zephir_init_properties_Sparrow_Encryption_Crypt;
	/**
	 * Defaults
	 */
	zephir_declare_class_constant_string(sparrow_encryption_crypt_ce, SL("DEFAULT_ALGORITHM"), "sha256");

	zephir_declare_class_constant_string(sparrow_encryption_crypt_ce, SL("DEFAULT_CIPHER"), "aes-256-cfb");

	/**
	 * Padding
	 */
	zephir_declare_class_constant_long(sparrow_encryption_crypt_ce, SL("PADDING_ANSI_X_923"), 1);

	zephir_declare_class_constant_long(sparrow_encryption_crypt_ce, SL("PADDING_DEFAULT"), 0);

	zephir_declare_class_constant_long(sparrow_encryption_crypt_ce, SL("PADDING_ISO_10126"), 3);

	zephir_declare_class_constant_long(sparrow_encryption_crypt_ce, SL("PADDING_ISO_IEC_7816_4"), 4);

	zephir_declare_class_constant_long(sparrow_encryption_crypt_ce, SL("PADDING_PKCS7"), 2);

	zephir_declare_class_constant_long(sparrow_encryption_crypt_ce, SL("PADDING_SPACE"), 6);

	zephir_declare_class_constant_long(sparrow_encryption_crypt_ce, SL("PADDING_ZERO"), 5);

	zend_class_implements(sparrow_encryption_crypt_ce, 1, sparrow_encryption_crypt_cryptinterface_ce);
	return SUCCESS;
}
...
PHP_METHOD(Sparrow_Encryption_Crypt, __construct)
{
	...
	ZEND_PARSE_PARAMETERS_START(0, 3)
		...
		Z_PARAM_OBJECT_OF_CLASS_OR_NULL(padFactory, zephir_get_internal_ce(SL("sparrow\\encryption\\crypt\\padfactory")))
	...
	if (Z_TYPE_P(padFactory) == IS_NULL) {
		...
		object_init_ex(padFactory, sparrow_encryption_crypt_padfactory_ce);
...
PHP_METHOD(Sparrow_Encryption_Crypt, decrypt)
{
	...
	if (1 == ZEPHIR_IS_EMPTY(&decryptKey)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(sparrow_encryption_crypt_exception_exception_ce, "Decryption key cannot be empty", "sparrow/Encryption/Crypt.zep", 175);
		return;
	}
	...
	if (ZEPHIR_IS_TRUE_IDENTICAL(&_7)) {
		...
		if (!ZEPHIR_IS_IDENTICAL(&digest, &_8$$7)) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(sparrow_encryption_crypt_exception_mismatch_ce, "Hash does not match.", "sparrow/Encryption/Crypt.zep", 224);
			return;
		}
	}
...
PHP_METHOD(Sparrow_Encryption_Crypt, encrypt)
{
	...
	if (1 == ZEPHIR_IS_EMPTY(&encryptKey)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(sparrow_encryption_crypt_exception_exception_ce, "Encryption key cannot be empty", "sparrow/Encryption/Crypt.zep", 282);
		return;
	}
	zephir_read_property(&_0, this_ptr, ZEND_STRL("cipher"), PH_NOISY_CC | PH_READONLY);
	ZEPHIR_CPY_WRT(&cipher, &_0);
	...
	if (ZEPHIR_IS_FALSE_IDENTICAL(&iv)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(sparrow_encryption_crypt_exception_exception_ce, "Cannot calculate Random Pseudo Bytes", "sparrow/Encryption/Crypt.zep", 295);
		return;
	}
...
PHP_METHOD(Sparrow_Encryption_Crypt, checkCipherHashIsAvailable)
{
	...
	if (1 != zephir_array_isset(&available, &lower)) {
		...
		object_init_ex(&_1$$5, sparrow_encryption_crypt_exception_exception_ce);
		ZEPHIR_INIT_VAR(&_2$$5);
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(&_1$$5, "sparrow/Encryption/Crypt.zep", 608);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Sparrow_Encryption_Crypt, cryptPadText)
{
	...
	if (ZEPHIR_IS_TRUE_IDENTICAL(&_0)) {
		...
		if (_4$$3) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(sparrow_encryption_crypt_exception_exception_ce, "Padding size cannot be less than 0 or greater than 256", "sparrow/Encryption/Crypt.zep", 641);
			return;
		}
...
PHP_METHOD(Sparrow_Encryption_Crypt, decryptGcmCcmAuth)
{
	...
	if (ZEPHIR_IS_FALSE_IDENTICAL(&decrypted)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(sparrow_encryption_crypt_exception_exception_ce, "Could not decrypt data", "sparrow/Encryption/Crypt.zep", 782);
		return;
	}
	RETURN_CCTOR(&decrypted);
}
...
PHP_METHOD(Sparrow_Encryption_Crypt, encryptGcmCcm)
{
	...
	if (ZEPHIR_IS_TRUE_IDENTICAL(&_1)) {
		...
		if (1 == ZEPHIR_IS_EMPTY(&authData)) {
			ZEPHIR_THROW_EXCEPTION_DEBUG_STR(sparrow_encryption_crypt_exception_exception_ce, "Auth data must be provided when using AEAD mode", "sparrow/Encryption/Crypt.zep", 841);
			return;
		}
	...
	if (ZEPHIR_IS_FALSE_IDENTICAL(&encrypted)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(sparrow_encryption_crypt_exception_exception_ce, "Could not encrypt data", "sparrow/Encryption/Crypt.zep", 870);
		return;
	}
...
PHP_METHOD(Sparrow_Encryption_Crypt, initializeAvailableCiphers)
{
	...
	zephir_check_call_status();
	if (!ZEPHIR_IS_TRUE_IDENTICAL(&_0)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(sparrow_encryption_crypt_exception_exception_ce, "This class requires the openssl extension for PHP", "sparrow/Encryption/Crypt.zep", 892);
		return;
	}
	...
	array_init(&allowed);
	zephir_is_iterable(&available, 0, "sparrow/Encryption/Crypt.zep", 909);
...
PHP_METHOD(Sparrow_Encryption_Crypt, getIvLength)
{
	...
	zephir_check_call_status();
	if (ZEPHIR_IS_FALSE_IDENTICAL(&length)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(sparrow_encryption_crypt_exception_exception_ce, "Cannot calculate the initialization vector (IV) length of the cipher", "sparrow/Encryption/Crypt.zep", 962);
		return;
	}
	RETURN_CCTOR(&length);
}

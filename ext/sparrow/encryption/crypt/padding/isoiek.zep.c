...
{
	ZEPHIR_REGISTER_CLASS(Sparrow\\Encryption\\Crypt\\Padding, IsoIek, sparrow, encryption_crypt_padding_isoiek, sparrow_encryption_crypt_padding_isoiek_method_entry, 0);

	zend_class_implements(sparrow_encryption_crypt_padding_isoiek_ce, 1, sparrow_encryption_crypt_padding_padinterface_ce);
	return SUCCESS;
}
...
PHP_METHOD(Sparrow_Encryption_Crypt_Padding_IsoIek, unpad)
{
	...
	while (1) {
		_0 = counter > 0;
		if (_0) {
			zephir_array_fetch_long(&_1, &inputArray, counter, PH_NOISY | PH_READONLY, "sparrow/Encryption/Crypt/Padding/IsoIek.zep", 48);
			ZVAL_LONG(&_2, 0);
	...
	zephir_array_fetch_long(&_6, &inputArray, counter, PH_NOISY | PH_READONLY, "sparrow/Encryption/Crypt/Padding/IsoIek.zep", 55);
	ZVAL_LONG(&_2, 0x80);

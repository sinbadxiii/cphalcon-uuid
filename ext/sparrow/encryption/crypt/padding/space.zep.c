...
{
	ZEPHIR_REGISTER_CLASS(Sparrow\\Encryption\\Crypt\\Padding, Space, sparrow, encryption_crypt_padding_space, sparrow_encryption_crypt_padding_space_method_entry, 0);

	zend_class_implements(sparrow_encryption_crypt_padding_space_ce, 1, sparrow_encryption_crypt_padding_padinterface_ce);
	return SUCCESS;
}
...
PHP_METHOD(Sparrow_Encryption_Crypt_Padding_Space, unpad)
{
	...
	while (1) {
		_0 = counter >= 0;
		if (_0) {
			zephir_array_fetch_long(&_1, &inputArray, counter, PH_NOISY | PH_READONLY, "sparrow/Encryption/Crypt/Padding/Space.zep", 48);
			ZVAL_LONG(&_2, 32);

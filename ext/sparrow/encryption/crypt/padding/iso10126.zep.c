...
PHP_METHOD(Sparrow_Encryption_Crypt_Padding_Iso10126, pad)
{
	...
	if (_1 <= _2) {
		while (1) {
			...
			ZEPHIR_CALL_FUNCTION(&_3$$3, "rand", &_4, 46);
			zephir_check_call_status();
			ZEPHIR_CALL_FUNCTION(&_5$$3, "chr", &_6, 43, &_3$$3);
			zephir_check_call_status();
			zephir_concat_self(&padding, &_5$$3);
		}
	}
	ZVAL_LONG(&_7, paddingSize);
	ZEPHIR_CALL_FUNCTION(&_8, "chr", &_6, 43, &_7);
	zephir_check_call_status();
...
PHP_METHOD(Sparrow_Encryption_Crypt_Padding_Iso10126, unpad)
{
	...
	ZEPHIR_RETURN_CALL_FUNCTION("ord", NULL, 45, &last);
	zephir_check_call_status();
	RETURN_MM();
}

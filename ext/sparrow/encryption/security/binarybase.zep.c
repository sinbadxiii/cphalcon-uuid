...
{
	ZEPHIR_REGISTER_CLASS(Sparrow\\Encryption\\Security, BinaryBase, sparrow, encryption_security_binarybase, sparrow_encryption_security_binarybase_method_entry, 0);
...
PHP_METHOD(Sparrow_Encryption_Security_BinaryBase, number)
{
	...
	if (UNEXPECTED(len <= 0)) {
		ZEPHIR_THROW_EXCEPTION_DEBUG_STR(sparrow_encryption_security_exception_ce, "Input number must be a positive integer", "sparrow/Encryption/Security/BinaryBase.zep", 207);
		return;
	}
	ZVAL_LONG(&_0, 0);
	ZVAL_LONG(&_1, len);
...
PHP_METHOD(Sparrow_Encryption_Security_BinaryBase, base)
{
	...
	zephir_check_call_status();
	zephir_is_iterable(&bytes, 0, "sparrow/Encryption/Security/BinaryBase.zep", 241);
